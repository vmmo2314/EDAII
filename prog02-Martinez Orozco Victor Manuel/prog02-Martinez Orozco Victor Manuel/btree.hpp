...
aux = r->getData();
cout << aux.toStringSinSeparadores() << "\n";
...
nAux = findData(nombre);
    ...
    ///validaciones en caso de que el nodo a eliminar sea la raíz
        ...
        cout << "Nodo padre: " <<padreAux->getData().toStringSinSeparadores()<<endl;  //Si el nodo no a eliminar no es igual a la raíz, sí tiene padre y es posible mostrar sus datos
        }
    ...
    //esto para no afectar  el religado de datos
        ...
        ///Eliminación de un nodo con un hijo por la izquierda
        ...
        ///Eliminación de un nodo con un hijo por la derecha
...
nAux = findData(Nombre);
    ...
    if(nAux == root){
        cout << "Nodo raiz, el padre es el mismo elemento...\n" <<nAux->getData().toStringSinSeparadores()<<endl;
    }
    else{
        cout << "Nodo padre: " <<nAux->getPadre()->getData().toStringSinSeparadores()<<endl;
    }
}
